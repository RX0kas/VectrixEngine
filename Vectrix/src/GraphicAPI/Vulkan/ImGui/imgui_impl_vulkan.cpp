...
static void ImGui_ImplVulkan_CreateShaderModules(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    auto shaders = CompileImGuiShader();
    auto vert = shaders.first;
    auto frag = shaders.second;
    if (bd->ShaderModuleVert == VK_NULL_HANDLE)
    {
        VkShaderModuleCreateInfo default_vert_info = {};
        default_vert_info.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        default_vert_info.codeSize = vert.size() * sizeof(uint32_t);
        default_vert_info.pCode = vert.data();
    ...
    if (bd->ShaderModuleFrag == VK_NULL_HANDLE)
    {
        ...
        default_frag_info.codeSize = frag.size() * sizeof(uint32_t);
        default_frag_info.pCode = frag.data();
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info)
{
    ...
    io.BackendRendererName = "imgui_impl_vulkan_vc";
...
VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
{
    ...
    ImGui_ImplVulkan_WriteTextureDescriptor(descriptor_set, sampler, image_view, image_layout);
...
VkDescriptorSetLayout ImGui_ImplVulkan_GetTextureDescriptorSetLayout()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    IM_ASSERT(bd != nullptr && bd->DescriptorSetLayout != VK_NULL_HANDLE);
    return bd->DescriptorSetLayout;
}

void ImGui_ImplVulkan_WriteTextureDescriptor(VkDescriptorSet descriptor_set, VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
